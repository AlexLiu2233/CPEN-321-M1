...
class ProfileRepositoryImpl @Inject constructor(
    ...
    override suspend fun deleteProfile(): Result<Unit> {
        return try {
            // Auth header handled by your interceptor; keep the "" param for consistency with your existing calls
            val response = userInterface.deleteProfile("")
            if (response.isSuccessful) {
                // Backend likely returns 204 No Content—this path covers that too
                // Clean up local auth immediately after successful delete
                tokenManager.clearToken()
                RetrofitClient.setAuthToken(null)
                Result.success(Unit)
            } else {
